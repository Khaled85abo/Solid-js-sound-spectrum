...
const RadialGraph: Component<{
  ...
  const computed = createMemo(() => {
    const data = rawData();
    const total = data.reduce((pre, curr) => pre + curr, 0);
    const highCount = data.filter(d => d > 32).length;
    const intesity = highCount / data.length;
    const paths: {
      path: string;
      color: string;
    }[] = []
    const range = 1.0 + intesity;
    ...
    return { paths, intesity };
  });

  return (
    <g transform={`scale(${computed().intesity * scale + 1})`}>
